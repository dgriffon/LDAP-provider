 ...
 */
public class UsersNameClassPairCallbackHandler implements NameClassPairCallbackHandler {

    private static Logger logger = LoggerFactory.getLogger(UsersNameClassPairCallbackHandler.class);

    private LDAPUserGroupProvider ldapUserGroupProvider;
    private List<String> names = new ArrayList<String>();
    private List<LDAPUserCacheEntry> cacheEntries  = new ArrayList<>();
    private String key;

    public UsersNameClassPairCallbackHandler(LDAPUserGroupProvider ldapUserGroupProvider, String key) {
    ...
    public List<LDAPUserCacheEntry> getCacheEntries() {
        return cacheEntries;
    }
    ...
    public void handleNameClassPair(NameClassPair nameClassPair) throws NamingException {
        if (nameClassPair instanceof SearchResult) {
            ...
            LDAPUserCacheEntry cacheEntry = ldapUserGroupProvider.getLdapCacheManager().getUserCacheEntryByDn(key, searchResult.getNameInNamespace());
            if (cacheEntry == null || cacheEntry.getExist() ==  null || !cacheEntry.getExist()) {
                UserNameClassPairCallbackHandler nameClassPairCallbackHandler = new UserNameClassPairCallbackHandler(ldapUserGroupProvider, cacheEntry, key);
                nameClassPairCallbackHandler.handleNameClassPair(nameClassPair);
                cacheEntry = nameClassPairCallbackHandler.getCacheEntry();
                if (cacheEntry != null) {
                    ldapUserGroupProvider.getLdapCacheManager().cacheUser(key, cacheEntry);
                }
            }
            if (cacheEntry != null) {
